...
//console.log(socket);
    ...
    socket.on('getTopics', function(data) {
      ...
      logger.info('url?',config.wundergroundGeoLookupURLPartial+data.lat+','+data.long+'.json');
       api.getRest(config.wundergroundGeoLookupURLPartial+data.lat+','+data.long+'.json').then(function(data){

          logger.info('##HEYO i got a WU response',data);
